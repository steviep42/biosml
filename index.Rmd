...
names(myl) <- c("err","means","trees")
```

Look at this data frame which has a compute error / validation rate for each set of trees that we create:

```{r ploterr}
plot(y=myl$err,x=myl$trees,type="l",
     xlab="Number Of Trees",ylab="Error",main="Error Rate")
grid()
```
...
The randomForest function will generate "importance" information on the variables. This information is considered as the method makes its splits.

```{r importrf}
varImpPlot(non_caret_rf)
```


To get an idea about how the model performs with other values of **mtry** we could write a function to accept this value as an argument. Our function will return the RMSE corresponding to the supplied value of **mtry**. 
...
```{r printcaretrf}
set.seed(321)
caret_rf <- train(mpg ~ .,data=new_train,method="rf")
caret_rf
```

As a matter of fact we can plot this model with not a lot of effort and it will show us something interesting. An mtry of 6 seems to be the best. This is the power of methods that use hyperparamters. If we move through a number of values for the **mtry** then perhaps we can find the best value to get the best result. 
...
It looks like a value of 5 will produce the lowest value for RMSE. 
...
cat("RMSE for svm is: ",Metrics::rmse(example_Test$mpg,svm_preds),"\n")
```
...
Metrics::rmse(example_Test$mpg,predict(comp_mod_glmnet,example_Test))
...
```{r imptree}
rf_mpg <- randomForest(mpg ~ .,data=mtcars, importance=TRUE)
importance(rf_mpg,type=1)
```

This gives us some clues as to what variables we might want to include or exclude in our formula. 
...
```{r plotde}
DataExplorer::plot_correlation(correlations)
```
...
If you look at the summary of the model it looks pretty bleak because most of the coefficients aren't significant. You wouldn't want to take your career on this model. 
...
```{r mtsapplyna}
(miss <- sapply(mtcars_na,function(x) sum(is.na(x))))
barplot(rev(sort(miss)),main="Missingness Of Data")
