...
Since we are predicting a numeric outcome we would like to come up with a metric to help us fogure out if the model we havbe is good or not. With classification situations we canconfusion matrices and ROC curves. Here we will ue something more simplistic but effective - Root Mean Square Error. The formula looks like the following where P represents a vector of predictions and O represents a vector of the observed (true) values.
...
Now we look at bagged trees which involves looking at looks at many trees in average - this is an ensemble method. It helps to reduce the variance associated with decision trees which can be highly sensitive to changes in data. The term bagging refers to "boostrap aggregation". To review, the Decision Tree process can be represented like this:
