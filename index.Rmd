...
So let's plot the training data as well as the resulting regression line (in the color red) coming from the lm object. We'll also plot the results from the Support Vector Machine predictions on the same graph (in blue). From this plot it appears that the Support Vector Machine does a better job of "following" the actual data - at least for the training data. We might even be able to improve the SVM performance if we tune the hyperparameters but the default without tuning seems better than the lm.  It's this type of observation that leads one to consider if there are yet other methods that would result in even better results.  
...
points(svm_preds~V1,data=pdf[order(pdf$V1),],col="blue",pch=4,type="l")
```
...
Let's keep going. We'll use some more methods to see how they perform on the same data. 
