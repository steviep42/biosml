...
While this process was useful the resulting confusion matrix corresponded to just *ONE* particular threshold ? What if we had picked another value ? We would then get a different confusion matrix as well as different performance measures. In effect we would have to repeat steps 3-6 all over again !!! 
...
```{r}
caret::confusionMatrix(glm_label_preds,
                       glm_test$diabetes,positive="pos")
```
...
We can explore any number methods, implement K Fold Cross Validation,and get feedback on the performance measures all at the same time. Let's reframe our above work using the caret package conveniences. We want to use Cross Fold validation here. 
...
So we get an estimate of a 75% accuracy rate when the model is applied to out of sample data. This isn't so impressive but we aren't here to solve that problem. So let's make some predictions use thing test data to see what the Accuracy rate is. 

```{r pmglmlabs}
pm_glm_pred_labels <- predict(pm_glm_mod,test)

confusionMatrix(pm_glm_pred_labels,
                test$diabetes,
                positive = "pos")
```
...
caTools::colAUC(probs,test$diabetes,plotROC = T)
```
