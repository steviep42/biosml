...
A good example of explanatory work is the often referred to "Titanic" dataset that has lots of information on the passengers who travelled on the ill-fated Titanic ship. This data set is used in many Data Science type educational contexts to talk about building models.
...
Since we are attmepting to predict a binary outcome here ("pos" or "neg") we'll need to use something other than linear regression which is used to predict numeric outcomes. We'll go with Logistic Regression as it is a tried and true method for doing this type of thing. 
...
So now we have our predictions in terms of actual labels that we could then use to compare to the actual labels that are stored in the "diabetes" column of the test data frame. This table provdes the basis for computing a number of performance measures such as accuracy, precision, sensitivity, specificity and others. In predictive modeling we are always interested in how well any given model will perform on "new" data.
...
How precise is the model ? This is also known as Positive Predictive Value. We take the number of True Postives (TP) and divide that by the sum of True Positives (TP) and False Positives (FP). The denominator is the sum of row 2 in our matrix.
...
Let's find a way to generalize these steps. First, let's create a function that allows us to compute the True Positive Rate (aka "Sensitivity") and the False Postive Rate ( 1 - Specificity). If we apply it to our predictions from our example in progress, the output would be as follows. 
...
We could now use this function to compute these metrics for any set of predictions vs outcomes. We could generalize this function to accept an alpha so we could explore the full probability domain (0 - 1) and then plot the TPR vs FPR in effect creasting something known as a ROC Curve aka Receiver Operating Characteristic Curve.
...
Let's look at a large number of threshold / alpha values:
