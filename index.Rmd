...
NOTE: This document is in progress and is in *"draft status"*

This content exists for two main reasons:

1) I wish to motivate use of the **caret** package. Another way to say this is that I'm trying to "scare" you into using **caret** because it provides conveniences and capabilities that help you focus on building models over trying to understand the sometimes arcane syntax associated with the individual modeling functions in R. There is nothing wrong with being able to go deep into a specific method in terms of parameter tuning though you should first start learning how to build models after which we'll discuss appropriate assessment strategies.

Of course, there is absolutely no requirement for anyone to use the **caret** package but it does provide uniform access to over 200 modeling methods. All one needs to do is have some data, a metric by which to score (e.g. RMSE, AUC, logloss), and a method of interest. 

A cool feature of the **caret** package is that it provides access to methods for helping identify how well your selected model might apply to "out of sample" data. Additionally, you can see how well it comapres to other models. 

2) A second reason this content exists is to provide a rapid introduction to Machine Learning with R. It's better to get up and running with building some models than it is to sit back and read how to do it. The process of assessing models and diagnosing them can be an in depth process thus I don't wish to understate what it takes to do that. However, moving forward with some of the basic mechanics and concepts is good. One can always work with the individual methods or go to Section 6 to start with caret. The larger message is to get busy getting your hands dirty. 
...
This step involves taking the standard deviation of the vector / column. Then divide each valuein the vector / column by this computed standard deviation. We'll process the **centered** data from the previous computation.
...
We could do this on a data frame by using the **apply** function. The problme with this "do it yourself approach" is that we don't have an easy way to reverse the scaling and centering. 
...
There is a function called **scale** which will do this for us and in a way that will allow us to reverse those operations.

```{r}
(center_scaled <- scale(mtcars,center=TRUE,scale=TRUE))
```

If you look at the bottom of the above operation, there are two **attributes** that contain the information necessary to **reverse scale** the scaled data. Soif you do this yourself then you have to reverse scaled yourself. 
...
training_idx <- createDataPartition(mtcars$mpg, p =.80, list=FALSE)
...
To verify that the data is being centered and scaled, checkout the **myLm** object:
...
This is convenient because if we now wish to use the **predict** function, it will scale the **test** data that we provide on our behalf. What we get back will be in terms of the uncenter and unscaled predicted variable (mpg).
