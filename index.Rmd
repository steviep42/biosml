...
So then we look at different plots to become aquainted with the data:
...
While this is all very interesting, it’s doubtful that anything we learn  will save anyone’s life. It's unlikley that any conclusions we reach would apply to modern ship travel. However, we can experiment with different algorithms without consequence as we build models. Nonetheless, it remains very useful for educational purposes
...
```{r pairsnumeric}
pairs(mtcars[,c(1,3:7)],main="Pairs plot for numeric features")
```
...
```{r amplot}
mtcars %>% mutate(cyl=factor(cyl),am=factor(am,labels=c("Manual","Auto"))) %>% 
...
```{r tabplotbar}
mtcars %>% mutate(am=factor(am,labels=c("Automatic","Mabual")),
                  cyl=factor(cyl)) %>% 
                  count(am,cyl) %>%  
...
Let's check out the [XGBoost](https://xgboost.readthedocs.io/en/latest/) function which is an algorithm that has enjoyed recent popularity and celebrity in the world of Data Science. XGBoost is an implementation of gradient boosted decision trees designed for speed and performance. XGBoost requires us to specify the input as matrices. We also have to give it the input in the form of X, Y arguments where the first is a matrix of all the predictor variables and the latter is the thing being predictd. In this case the MPG which is the first column


```{r xgbooster, message=FALSE}
library(xgboost)
xgb <- xgboost(data = as.matrix(train_df[,-1]),label=train_df[,1],nrounds=25,verbose=0)
```
...
Tree-based methods employ a segmentation strategy that partitions the feature / predictor space into a series of decsisions which has the added benefit of being easy to understand. Think of it as a flow chart for making decsions. The viewer of the chart is presented with a diagram that offers outcomes in response to decsions about important predictors found in the data set. 
...
So you just learned that with a decision tree, you do not need to scale data which means you can get started quickly. We won't generate a train / test pair here since we are just trying to understand the structure. 

We can easily plot the tree emanating from the **rpart** comman but it can also easily be **read** from the basic output. 


```{r mtcrpart}
tree_simple <- rpart(factor(am) ~ .,
                     data=mtcars,
                     method="class")
tree_simple
...
The associated plot represents this graphically while also showing us the distribution of, in this case, 0 outcomes vs 1 outcomes. In the parlance of decision trees we have the **root node** at the top followed by some leaf nodes which also happen to be terminal nodes. The nodes are connected by branches which indicate the flow of the decision which in this case is quite simple. It's also easy to show some one a tree and they don't need to know any of the underlying workings of the method which is why this is quite useful. 
...
Perhaps you noticed that in the example I included an argument called **cp** which corresponds to a "complexity parameter" which influences how the tree splits at various nodes. 
...
![](/Users/esteban/GIT_REPOS_DIR/biosml/pics/rpartctrol2.png)
...
![](/Users/esteban/GIT_REPOS_DIR/biosml/pics/tree2.png)
...
The bootstrap method of sampling will resample the training data some number of times (with replacement) and retrain a number of models on the resampled data to average out the error. This looks something like:
 ...
 ![](/Users/esteban/GIT_REPOS_DIR/biosml/pics/bagged.png)
...
![](/Users/esteban/GIT_REPOS_DIR/biosml/pics/bagging2.png)
...
In Random Forest, only a subset of features are selected at random at each split in a decision tree. In bagging, all features are used. You could think of random forests as being an extension of Bagged Trees. Typically they are also an improvement. Even though the method represents a collection of trees we still get the added advantage of not having to pre process the data. 

This doesn't mean that we shouldn't, for example, try to see if the data cannot be reduced in terms of dimenisionality. Nor does it mean that if we do scale the data that we can't use it with Tree methods. Basically, one of the main advantages of the method is that it tolerates a combination of data types on different scales which makes it good as a "go to" method.
