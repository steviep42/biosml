...
So we get an estimate of a 77% accuracy rate when the model is applied to out of sample data. This isn't so impressive but we aren't here to solve that problem. So let's make some predictions use thing test data to see what the Accuracy rate is. 
...
The function we will use here, **ranger**, has three hyper parameters which could be set to a range of values which, in turn, could influence the resulting model. With glm, we didn't really have a hyper parameter. We'll worry about hyper parameters later. 
